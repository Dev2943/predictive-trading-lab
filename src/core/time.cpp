...
std::string_view to_string(Stage s) noexcept {
    switch (s) {
        case Stage::ExchangeTime:
            return "exchange_time";
        case Stage::ReceiveTime:
            return "receive_time";
        case Stage::FeatureEndTime:
            return "feature_end_time";
        case Stage::DecisionTime:
            return "decision_time";
        case Stage::SubmittedTime:
            return "submitted_time";
        case Stage::ArrivalTime:
            return "arrival_time";
        case Stage::FillTime:
            return "fill_time";
        case Stage::AckTime:
...
std::string ChainViolation::describe() const {
    ...
    out +=
        (rule == ChainRule::StrictlyAfter) ? " must be strictly after " : " must be at or after ";
...
std::optional<ChainViolation> validate_chain(const LifecycleTimes& t) noexcept {
    ...
    if (is_set(t.decision_time) && is_set(t.arrival_time) && t.arrival_time <= t.decision_time) {
...
bool check_chain(const LifecycleTimes& t, const char* file, int line) {
    ...
    // Counted in BOTH configurations. In debug the throw follows, but the
    // counter is what a release run reports in its manifest, and a test that
    // catches the exception still wants to observe that it was recorded.
    ...
    return false;
#else
    std::string msg = "point-in-time chain violation at ";
    msg += file;
    msg += ':';

...
bool parse_date(std::string_view text, Timestamp& out) noexcept {
    // Exactly ten characters. A trailing time component is a units error, not
    // a generosity opportunity.
    if (text.size() != 10) return false;
    return parse_timestamp(text, out);
}

bool parse_time_of_day(std::string_view text, Duration& out) noexcept {
    using namespace std::chrono;
    ...
    int mm = 0;
    int ss = 0;
    if (!parse_uint(text, 0, 2, hh)) return false;
    if (!parse_uint(text, 3, 2, mm)) return false;
    if (!parse_uint(text, 6, 2, ss)) return false;
    // 24:00:00 is rejected: a session boundary at the end of a day should be
    // expressed as the next day, not as an hour that does not exist.
    if (hh > 23 || mm > 59 || ss > 59) return false;

    out = hours{hh} + minutes{mm} + seconds{ss};
    return true;
}

...
Result<std::vector<oms::Fill>> BrokerSimulator::on_market(InstrumentId instrument,
    ...
    for (auto it = pending_.begin(); it != pending_.end();) {
        ...
        if (fill_cfg_.respect_displayed_size && state.has_quote) {
            const double displayed =
                side == Side::Buy ? state.ask_size.get() : state.bid_size.get();
            if (displayed > 0.0 && displayed < allowed) {
                allowed = displayed;
                capped = true;
        ...
        } else if (state.interval_volume.get() <= 0.0) {
            // No volume traded in this interval: no liquidity was available.
            // Zero participation means NO FILL, not an unconstrained one.
            ++it;
            continue;
        }

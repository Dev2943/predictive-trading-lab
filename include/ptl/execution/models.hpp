...
struct MarketState {
    ...
    /// Whether `interval_volume` is a MEASUREMENT or simply unset.
    ///
    /// Zero is ambiguous on its own: it means "nothing traded this
    /// interval" when a bar supplied it, and "no bar was involved" on a
    /// quote-only update. The first must block a fill; the second must not,
    /// or quote-driven execution could never fill at all. Defaults true so
    /// every bar-sourced state keeps its Phase 3 meaning.

...
// ---------------------------------------------------------------------------
// PTL_CHECK_CHAIN -- the enforcement macro
// ---------------------------------------------------------------------------
//
// validate_chain() detects a violation. This decides what to DO about one, in
// exactly one place, so that Phase 3 does not invent its own policy and Phase 8
// a third.
//
// Policy:
//   debug   -- throw. A violated chain means the point-in-time guarantee is
//              void, so every number the run produces afterwards is fiction.
//              Failing loudly at the first violation is strictly more useful
//              than a correct-looking equity curve.
//   release -- count and continue. A long backtest should not die at hour three
//              over one bad record, but the count is written into the run
//              manifest and a non-zero value INVALIDATES the run. The result is
//              still produced; it is simply marked untrustworthy.
//
// Cost: validate_chain() benchmarks at roughly 30 ns. That is fine per ORDER
// LIFECYCLE, which is where this belongs. It is not fine per market event, and
// there is no legitimate reason to call it there -- a market event has an
// EventTime, not a LifecycleTimes. Use EventTime::ok() for those.
//
// Set PTL_ENABLE_CHAIN_CHECKS=0 to compile the check out entirely. That is a
// measurement tool for Phase 13, not a production setting: turning off the
// invariant to make a benchmark look better is exactly the self-deception this
// project exists to avoid.

#if !defined(PTL_ENABLE_CHAIN_CHECKS)
#define PTL_ENABLE_CHAIN_CHECKS 1
#endif

namespace detail {

/// \returns true when the chain is well-ordered.
/// \throws std::logic_error in debug builds on violation.
bool check_chain(const LifecycleTimes& t, const char* file, int line);

}  // namespace detail

}  // namespace ptl

// Macros are preprocessor constructs and belong outside the namespace.
#if PTL_ENABLE_CHAIN_CHECKS
#define PTL_CHECK_CHAIN(times_) ::ptl::detail::check_chain((times_), __FILE__, __LINE__)
#else
#define PTL_CHECK_CHAIN(times_) (true)
#endif

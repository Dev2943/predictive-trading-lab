...
TEST_CASE("PTL_CHECK_CHAIN accepts a well-ordered chain silently", "[core][pit][invariant]") {
    reset_chain_violation_count();
    REQUIRE(PTL_CHECK_CHAIN(reference_chain()));
    REQUIRE(chain_violation_count() == 0);
}

TEST_CASE("PTL_CHECK_CHAIN enforces the documented debug/release policy",
          "[core][pit][invariant][leakage]") {
    // Review finding M-3. validate_chain() DETECTS; this decides what to do,
    // in one place, so Phase 3 does not invent its own policy and Phase 8 a
    // third.
    //
    //   debug   -> throw. The point-in-time guarantee is void, so every number
    //              produced afterwards is fiction. A correct-looking equity
    //              curve is worse than a crash.
    //   release -> count and continue. A long backtest should not die at hour
    //              three over one record, but the count enters the manifest and
    //              a non-zero value INVALIDATES the run.
    reset_chain_violation_count();

    LifecycleTimes same_bar = reference_chain();
    same_bar.arrival_time = same_bar.decision_time;

#if defined(NDEBUG)
    REQUIRE_FALSE(PTL_CHECK_CHAIN(same_bar));
#else
    REQUIRE_THROWS_AS(PTL_CHECK_CHAIN(same_bar), std::logic_error);
#endif

    // Counted in BOTH configurations. A release run reports this in its
    // manifest; a debug test that catches the throw still wants to see it.
    REQUIRE(chain_violation_count() == 1);
    reset_chain_violation_count();
}

TEST_CASE("PTL_CHECK_CHAIN reports the offending file and line", "[core][pit][invariant]") {
#if !defined(NDEBUG)
    reset_chain_violation_count();
    LifecycleTimes bad = reference_chain();
    bad.fill_time = bad.arrival_time - std::chrono::nanoseconds{1};
    try {
        PTL_CHECK_CHAIN(bad);
        FAIL("expected a throw");
    } catch (const std::logic_error& e) {
        const std::string what{e.what()};
        // Locating the call site matters: by Phase 8 there will be many.
        REQUIRE(what.find("test_time_chain.cpp") != std::string::npos);
        REQUIRE(what.find("fill_time") != std::string::npos);
    }
    reset_chain_violation_count();
#endif
}

TEST_CASE("violation counts accumulate across many checks", "[core][pit][invariant]") {
    // The release path must keep counting rather than latching at one: the
    // manifest reports how bad it was, not merely that it happened.
    reset_chain_violation_count();
    LifecycleTimes bad = reference_chain();
    bad.arrival_time = bad.decision_time;

    for (int i = 0; i < 3; ++i) {
#if defined(NDEBUG)
        (void)PTL_CHECK_CHAIN(bad);
#else
        REQUIRE_THROWS(PTL_CHECK_CHAIN(bad));
#endif
    }
    REQUIRE(chain_violation_count() == 3);
    reset_chain_violation_count();
}

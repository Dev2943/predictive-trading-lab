...
TEST_CASE("parse_date rejects anything with a time component", "[core][time][config]") {
    // parse_timestamp deliberately accepts both forms, which is right for event
    // data. A configured DATE is different: silently accepting a datetime hides
    // a units error in a field that must be exactly right before anyone looks
    // at data.
    Timestamp ts{};
    REQUIRE(parse_date("2024-09-01", ts));
    REQUIRE(to_date_string(ts) == "2024-09-01");

    REQUIRE_FALSE(parse_date("2024-09-01T00:00:00Z", ts));
    REQUIRE_FALSE(parse_date("2024-09-1", ts));
    REQUIRE_FALSE(parse_date("2024-13-01", ts));
    REQUIRE_FALSE(parse_date("2024-02-30", ts));
    REQUIRE_FALSE(parse_date("", ts));
}

TEST_CASE("parse_time_of_day yields an offset from midnight", "[core][time][config]") {
    Duration d{};
    REQUIRE(parse_time_of_day("09:30:00", d));
    REQUIRE(d == std::chrono::hours{9} + std::chrono::minutes{30});
    REQUIRE(parse_time_of_day("16:00:00", d));
    REQUIRE(d == std::chrono::hours{16});
    REQUIRE(parse_time_of_day("00:00:00", d));
    REQUIRE(d == Duration::zero());

    REQUIRE_FALSE(parse_time_of_day("9:30:00", d));   // not zero-padded
    REQUIRE_FALSE(parse_time_of_day("09:30", d));     // no seconds
    REQUIRE_FALSE(parse_time_of_day("24:00:00", d));  // an hour that does not exist
    REQUIRE_FALSE(parse_time_of_day("09:60:00", d));
    REQUIRE_FALSE(parse_time_of_day("09:30:60", d));
    REQUIRE_FALSE(parse_time_of_day("", d));
}
